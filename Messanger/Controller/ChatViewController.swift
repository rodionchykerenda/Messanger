...
class ChatViewController: MessagesViewController {
    ...
    private func setupChatService(){
        ...
        chatService?.connect()
        chatService?.setDelegate(delegate: self)
    }
    ...
    private func getEncryptedMessage(text: String) -> String {
        let cryptographer = Cryptographer()
        let encryptedMessage = cryptographer.encrypt(text: text)
        return "\(cryptographer.getKey())\n\(encryptedMessage)"
    }
}
